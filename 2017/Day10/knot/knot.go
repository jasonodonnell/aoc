...
func (k *Knot) Hash() string {
	...
	for i < len(k.List) {
		...
		denseHash := int64(k.List[i] ^ k.List[i+1] ^
			k.List[i+2] ^ k.List[i+3] ^ k.List[i+4] ^
			k.List[i+5] ^ k.List[i+6] ^ k.List[i+7] ^
			k.List[i+8] ^ k.List[i+9] ^ k.List[i+10] ^
			k.List[i+11] ^ k.List[i+12] ^ k.List[i+13] ^
			k.List[i+14] ^ k.List[i+15])
